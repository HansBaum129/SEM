...
library(MIIVsem)     # needed for model-implied IV analyses
library(nlsem)       # needed for latent interaction model
...
ML_ParameterEstimates<-parameterEstimates(fit_sem_ML, ci = TRUE, level = 0.95, standardized = T)
standardizedSolution(fit_sem_ML,ci = TRUE)
#store parameter estimates for later comparison with 2SLS
str(ML_ParameterEstimates)
Ml_2SLS_Comp<-as.data.frame(ML_ParameterEstimates[,c(1:3,5)])
Ml_2SLS_Comp<-Ml_2SLS_Comp[c(1:3,5:7,10,12:14),]
Ml_2SLS_Comp
...
################################################
## SECTION 4:
## Model-implied instrumental variable analysis
################################################

semalt <- '
PU=~PU4+PU1+PU2+PU3
PEOU=~PEOU4+PEOU1+PEOU2+PEOU3
BI=~BI1+BI2
PU~PEOU
BI~PEOU + PU
'
miivs(semalt)
miive(semalt, vmsdata)
miive(semalt, vmsdata, sarg.adjust = "holm")
miive(semalt, vmsdata, se = "boot", boot.ci = "perc")

# Fit Models
fit.standard <- miive(semalt, vmsdata)
fit.boot.1000 <- miive(semalt, vmsdata, se = "boot", bootstrap = 1000)

# Comparison of estimates between ML and 2SLS
Ml_2SLS_Comp <- data.frame(matrix(ncol = 10,nrow= 10))
colnames(Ml_2SLS_Comp)<-c("lhs-ml","op-ml","rhs-ml", "est-ml","lhs-2sls","op-2sls","rhs-2sls", "est-2sls","diff","%diff")

Ml_2SLS_Comp1<-as.data.frame(ML_ParameterEstimates[,c(1:3,5)])
Ml_2SLS_Comp1<-Ml_2SLS_Comp1[c(1:3,5:7,10,12:14),]
Ml_2SLS_Comp[,1:4]<-Ml_2SLS_Comp1

Ml_2SLS_Comp2 <- estimatesTable(fit.standard)[, c("lhs", "op", "rhs", "est")]
Ml_2SLS_Comp2 <- Ml_2SLS_Comp2[c(23:25,19:21,17,1,3,2),]
Ml_2SLS_Comp[,5:8]<-Ml_2SLS_Comp2

# difference between ML and 2SLS
Ml_2SLS_Comp[,9] <- Ml_2SLS_Comp[,4]-Ml_2SLS_Comp[,8]
# % difference between ML and 2SLS (relative to ML estimate)
Ml_2SLS_Comp[,10] <- Ml_2SLS_Comp[,9]/Ml_2SLS_Comp[,4]
...
################################################
## SECTION 5:
## Latent interaction between PEOU and PU
################################################
...
vmsdatanlsem <- as.matrix(vmsdata[ ,c(9,6:8,5,2:4,16,17)])
describe(vmsdatanlsem)
model1 <- lav2nlsem(sem)
set.seed(1004)
start <- runif(count_free_parameters(model1))
# original Klein and Moosbrugger method (doesn't work as well as in Mplus)
int1em <- em(model1, vmsdatanlsem, start, convergence=1e-01, max.iter = 400, m = 16, verbose=TRUE, hessian=hess)
summary(int1em)

set.seed(1002)
start <- runif(count_free_parameters(model1))
# alternative estimation procedure 
int1qml <- qml(model1, vmsdatanlsem, start)
summary(int1qml)

# specifying the model directly in nlsem
#model2 <- specify_sem(num.x = 8, num.y = 2, num.xi = 2, num.eta = 1,
#                      xi = "x1-x4,x5-x8", eta = "y1-y2", interaction = "eta1~xi1:xi2")
#set.seed(1004)
#start <- runif(count_free_parameters(model2))
#int2em <- em(model2, vmsdatanlsem, start, convergence=1e-01, verbose=TRUE)
#summary(int2em)
#int2qml <- qml(model2, vmsdatanlsem, start)
#summary(int2qml)
